...
public:
    ...
    void setCamera( std::shared_ptr< TControlledCamera > & _camera );

    std::shared_ptr< TControlledCamera > const & Camera();
    GLFWwindow* ptrWindow();

private:
    std::shared_ptr< TControlledCamera > & ptrCamera();
...
private:
    std::shared_ptr< TControlledCamera > m_camera = nullptr;
